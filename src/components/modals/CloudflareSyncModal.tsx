...
export default function CloudflareSyncModal() {
  ...
  return (
    <div
      onClick={(e) => {
        if (e.target === e.currentTarget) setIsCfModalOpen(false)
      }}
    ...
    >
      <div className="w-full max-w-3xl bg-white dark:bg-dark-panel rounded-2xl shadow-2xl border border-slate-200/80 dark:border-dark-border overflow-hidden flex flex-col max-h-[88vh]">
        {/* ================= 1. 模态框顶部 Header ================= */}
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-sm font-bold text-slate-900 dark:text-white">
                  Cloudflare 云端同步与自动备份
                </h3>
                {isConfigured ? (
                  <span className="flex items-center gap-1 text-[10px] font-semibold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/40 px-2 py-0.5 rounded-full border border-emerald-200/60 dark:border-emerald-800/40">
                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
                    已连接
                  </span>
                ) : (
                  <span className="text-[10px] font-medium text-slate-400 bg-slate-100 dark:bg-dark-hover px-2 py-0.5 rounded-full">
                    未接入
                  </span>
                )}
              </div>
              <p className="text-xs text-slate-400">
                多版本自动历史快照备份 · 云端灾备与多设备智能同步中心
              </p>
            </div>
          </div>
        ...
        {/* ================= 2. 标签页切换栏 ================= */}
        <div className="flex border-b border-slate-100 dark:border-dark-border px-4 bg-white dark:bg-dark-panel gap-1 sm:gap-2">
          <button
          ...
          >
            ...
            <span>自动备份与快照</span>
            {snapshots.length > 0 && (
              <span className="ml-0.5 px-1.5 py-0.2 text-[10px] font-mono rounded-full bg-orange-100 dark:bg-orange-950 text-orange-600 dark:text-orange-400">
                {snapshots.length}
              </span>
            )}
          </button>
          ...
          {activeTab === 'console' && (
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <div className="p-3 bg-slate-50 dark:bg-dark-sidebar/60 border border-slate-100 dark:border-dark-border rounded-xl">
                  <span className="text-[10px] text-slate-400 uppercase font-semibold">端到端加密</span>
                  <div className="text-xs font-semibold text-slate-800 dark:text-white mt-1 flex items-center gap-1">
                    {cfConfig.enableE2EE ? (
                      <>
                        <Shield className="w-3.5 h-3.5 text-emerald-500" />
                        <span className="text-emerald-600 dark:text-emerald-400">AES-256 已开启</span>
                      </>
                    ) : (
                ...
                <div className="p-3 bg-slate-50 dark:bg-dark-sidebar/60 border border-slate-100 dark:border-dark-border rounded-xl">
                  <span className="text-[10px] text-slate-400 uppercase font-semibold">最近同步</span>
                  <div className="text-xs font-mono text-slate-800 dark:text-white mt-1 truncate">
                    {cfConfig.lastSyncTime
                      ? new Date(cfConfig.lastSyncTime).toLocaleTimeString()
                      : '暂无记录'}
                  </div>
                </div>
              </div>
